...
- **DNS rebinding**: `assertSafeUrl` validates the hostname string, and `resolveAndCheckHost` then resolves the hostname via `dns.lookup({ all: true })` and rejects if **any** answer falls in the blocklist (loopback, RFC 1918, link-local, IPv6 ULA/link-local, AWS IMDS, IPv4-mapped IPv6 forms). The fetch is issued through an undici `Agent` whose `connect.lookup` returns the already-validated IP, so the socket layer never re-resolves and cannot be tricked into connecting to a different (rebound) address. Redirects are followed manually so each hop re-runs both the hostname and resolved-IP checks. Regression coverage lives in `fetch-posting.test.ts` under "DNS rebinding defense".
